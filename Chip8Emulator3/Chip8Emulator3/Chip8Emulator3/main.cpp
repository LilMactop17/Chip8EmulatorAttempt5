...
public:
	...
	void OP_8xyE(); //SHL Vx
	void OP_9xy0(); //Skip Next Instruction Vx, Vy
...
private:
	...
	Chip8Func table0[16]{};
	Chip8Func table8[16]{};
	Chip8Func tableE[256]{};
	Chip8Func tableF[256]{}; // see #3
...
Chip8::Chip8()
	:	randGen(static_cast<std::mt19937::result_type>(
...

{
	...
	for (auto& f : tableE) f = &Chip8::OP_NULL;  // 256 sized now
	for (auto& f : tableF) f = &Chip8::OP_NULL;

	// 2) Top-level decode by high nibble
	table[0x0] = &Chip8::Table0;   // 0x00E0 / 0x00EE
	...
	table[0xE] = &Chip8::TableE;   // Ex??
	table[0xF] = &Chip8::TableF;   // Fx??

	// 3) 0x00?? sub-table by low nibble
	table0[0x0] = &Chip8::OP_00E0; // 00E0 -> last nibble 0x0
	table0[0xE] = &Chip8::OP_00EE; // 00EE -> last nibble 0xE

	// 4) 0x8xy? sub-table by low nibble
	...
	// 5) 0xEx?? sub-table by low **byte**
	tableE[0x9E] = &Chip8::OP_Ex9E;
	tableE[0xA1] = &Chip8::OP_ExA1;

	// 6) 0xFx?? sub-table by low **byte**
...
void Chip8::TableE() { (this->*tableE[opcode & 0x00FFu])(); }  // index by full low byte
...
void Chip8::OP_8xy6() {
	...
	registers[0xF] = (registers[Vx] & 0x1u); //Isolating LSB

	registers[Vx] >>= 1; //right shift by 1
}
